...
contract Artifact is ERC721 {
    ...
    constructor(uint256 _fee) ERC721("0xTokens", "OXT") public {
    ...
    function publish(string memory tokenURI) external onlyContractOwner returns(uint256) {
         _tokenIds.increment();
        require(maxArtifactsCount >= _tokenIds.current(), 'No More Works Can Be Published');
    ...
    function updateFee(uint256 _newFee) external onlyContractOwner returns(bool) {
    ...
    function balanceOfContract() external onlyContractOwner view returns(uint256) {
        return address(this).balance;
    }

    // Only token holder can its physical print works with fee.
    function requestPrint(uint256 _tokenId) external payable onlyTokenHolder(_tokenId) returns(bool) {
        require(msg.value >= fee, 'Not Enough Fee Value');
        PrintHistory memory ph = PrintHistory(msg.sender, _tokenId, block.timestamp);
    ...
    function getPrintHistoriesByTokenId(uint256 _tokenId) external view returns(address[] memory, uint256 [] memory) {
        require(_exists(_tokenId), 'Invalid Token Id');
        PrintHistory[] memory hisotires = printHistoriesByTokenId[_tokenId];

        address[] memory addrs = new address[](hisotires.length);
        uint[] memory timestamps = new uint[](hisotires.length);

        for (uint i=0; i < hisotires.length; i++) {
    ...
    function getPrintHistoriesByHolderAddress(address holder) external view returns(uint256[] memory, uint[] memory) {
        PrintHistory[] memory hisotires = printHistoriesByAddress[holder];
        
        uint256[] memory tokenIds = new uint256[](hisotires.length);
        uint[] memory timestamps = new uint256[](hisotires.length);
